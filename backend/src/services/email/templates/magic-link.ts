...
export function renderMagicLink(data: MagicLinkTemplateData): { subject: string; html: string } {
  ...
  const scoreSection = data.overallScore !== undefined
    ? `<div style="background: #EEF2FF; border-radius: 6px; padding: 12px 16px; margin: 16px 0;">
        <span style="color: #4338CA; font-weight: 600; font-size: 14px;">Candidate Score: ${escapeHtml(data.overallScore)}%</span>
      </div>`
    : ''
  ...
  const notesSection = data.recruiterNotes
    ? `<div style="background: #FEF9C3; border-left: 4px solid #EAB308; padding: 12px 16px; margin: 16px 0; border-radius: 0 6px 6px 0;">
        <p style="color: #713F12; font-size: 14px; margin: 0;"><strong>Recruiter Notes:</strong> ${escapeHtml(data.recruiterNotes)}</p>
      </div>`
    : ''
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff;">
  ...
  <div style="padding: 32px;">
    ...
    <p style="color: #6B7280; font-size: 14px; margin: 0 0 24px;">Hi ${escapeHtml(data.interviewerName)}, an interview has been scheduled for your review.</p>
    ...
    <p style="color: #9CA3AF; font-size: 13px; margin: 24px 0 0;">If the button doesn't work, copy and paste this link: <a href="${interviewUrl}" style="color: #6366F1;">${escapeHtml(interviewUrl)}</a></p>
  </div>
