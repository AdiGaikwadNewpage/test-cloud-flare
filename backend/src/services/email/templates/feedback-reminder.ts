...
export function renderFeedbackReminder(data: FeedbackReminderTemplateData): { subject: string; html: string } {
  ...
  const feedbackUrl = `${safeHref(data.frontendUrl)}/interviews/${encodeURIComponent(data.interviewId)}`
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff;">
  ...
  <div style="padding: 32px;">
    ...
    <p style="color: #6B7280; font-size: 14px; margin: 0 0 24px;">Hi ${escapeHtml(data.interviewerName)}, your feedback is needed for a recently completed interview.</p>
    ...
    <div style="border: 1px solid #E5E7EB; border-radius: 8px; padding: 20px; margin: 16px 0;">
      ...
      <p style="color: #374151; margin: 8px 0;"><strong>Interview Date:</strong> ${escapeHtml(formatDateTime(data.scheduledAt))}</p>
    </div>
    ...
    <p style="color: #9CA3AF; font-size: 13px; margin: 24px 0 0;">If the button doesn't work, copy and paste this link: <a href="${feedbackUrl}" style="color: #6366F1;">${escapeHtml(feedbackUrl)}</a></p>
  </div>
